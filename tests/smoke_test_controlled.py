#!/usr/bin/env python3
"""冒烟测试（控制请求数量版本）：验证各数据下载模块的正确性。

测试策略：
1. 使用极少量股票（仅浦发银行 sh.600000）
2. 限制日期范围，减少数据量
3. 跳过分钟线等大数据量测试
4. 总请求数控制在 200 以内
"""
...
logger = setup_logging("smoke_test_controlled")

# 控制请求数量的配置
TEST_CODES = ["sh.600000"]  # 仅1只股票
TEST_START_DATE = "2024-01-01"
TEST_END_DATE = "2024-03-31"  # 仅3个月
...
def test_phase_6_stock_kline():
    """测试 6：A股K线（仅日线/周线/月线，不复权，限制日期范围）"""
    logger.info("\n" + "=" * 50)
    logger.info("  阶段 6: A股K线 (日线/周线/月线, 不复权, 限量)")
    logger.info("=" * 50)
...
def test_phase_8_reports():
    ...
    check_table_count("performance_express", min_rows=0)
    check_table_count("forecast_report", min_rows=0)
...
def main():
    logger.info("🚀 开始冒烟测试 (控制请求数量版本): 验证各数据下载模块")
    logger.info(f"⚠️  测试股票: {', '.join(TEST_CODES)}")
    logger.info(f"⚠️  日期范围: {TEST_START_DATE} ~ {TEST_END_DATE}")
    logger.info(f"⚠️  目标: 总请求数 < 200")

    # 跳过了分钟线测试（阶段11）和全股票列表测试（阶段10），以减少请求数
    tests = [
